...
export default class SVGCreator extends AnimationCreator {
   ...
   * @memberof SVGCreator
   */
  setPathAnimation(path: { appendChild: (arg0: Node) => void }, i: number) {
    const _options = this.options
    const animateDom = document.createElementNS(
    ...
    switch (_options.mode) {
      case "sync":
        ...
        break
      case "delay":
        animateDom.setAttributeNS(
          null,
          "begin",
          _options.delay * (i + 1) + "ms"
        )
        break
      case "onebyone":
        animateDom.setAttributeNS(
