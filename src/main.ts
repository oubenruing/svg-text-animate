...
export default class SVGTextAnimate {
   ...
   *
   */
  setFont(fontfile: any) {
    return new Promise((resove, reject) => {
      opentype.load(fontfile || this.fontfile, (err: any, openfont: any) => {
        if (err) {
  ...
  getBounding(paths: any[]) {
    ...
    const y2 = paths
      .reduce(
        (
          r: { getBoundingBox: () => { (): any; new (): any; y2: number } },
          c: { getBoundingBox: () => { (): any; new (): any; y2: number } }
        ) => {
   ...
   * @returns {DOM} svgDom
   */
  createSVGDom(text: any) {
    let svgDom = null
    let svgpath = ""
    const _div = document.createElement("div")
    const paths = this.font.getPaths(
      text,
      0,
    ...
    const svg = `<svg width="${box.x2 - box.x1 + strokeWidth}" 
            ...
            viewBox="${box.x1} ${box.y1} ${box.x2 + strokeWidth} ${
              box.y2 + strokeWidth
            }"
    ...
    paths.forEach((path: { toSVG: (arg0: number) => string }) => {
